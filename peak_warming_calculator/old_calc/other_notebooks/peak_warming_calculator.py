...
def peak_warming_calculator(consumption_discount=0.035, consumption_growth=0.02,
    ...
    years_forecast = create_years_array(last_historical_year, end_year)
    ...
    T_complete_initial, T_forecast_initial = create_T_initial(T_2019, T_historical, alpha, delta_T,
                                                                years_forecast_length)

    W = create_total_consumption(start_year, last_historical_year, years, consumption_growth)
    ...
    for iteration in range(num_of_iterations):
        ...
        # SCC_calculated = calculate_SCC_for_perturbed_years(T_TCRE, T_forecast_iteration, years_forecast_length, years_forecast,
        #                                               T_historical, T_complete_iteration, W, consumption_discount, k_s, years,
        ...
        for perturbed_year in range(len(years_of_perturbation)):
            ...
            cost = cost_of_perturbation(T_complete_iteration, T_perturbed, W, discount_function, gamma, D0)
            SCC = cost / (10 ** 9)
            if SCC < P_100:
                SCC_list.append(SCC)
            else:
        ...
        SCC_forecasted, P0 = forecast_SCC(SCC_calculated, years_forecast)
        ...
        T_complete_iteration = np.concatenate([T_historical, T_forecast_iteration[1:]])
...
def create_T_initial(T_2019, T_historical, alpha, delta_T, years_forecast_length):
    T_forecast_initial = T_2019 + (delta_T-T_2019) * (1 - np.exp(-alpha * np.arange(years_forecast_length)))
...
# calculate_SCC_for_perturbed_years(T_TCRE, T_forecast_iteration, years_forecast_length, years_forecast,
#                                                       T_historical, T_complete_iteration, W, consumption_discount, k_s, years,
#                                                       years_of_perturbation, gamma, D0, P_100)

# def calculate_SCC_for_perturbed_years(T_TCRE, T_forecast_iteration, years_forecast_length, years_forecast, T_historical,
#                                       T_complete_iteration, W, consumption_discount, k_s, years, years_of_perturbation,
#                                       gamma, D0, P_100):
#     SCC_list = []
#     for perturbed_year in range(len(years_of_perturbation)):
#         ## define perturbation
...
def create_T_perturbed(T_TCRE, T_forecast_iteration, years_forecast_length, years_forecast, T_historical, k_s, perturbed_year,
                       years_of_perturbation):
    T_p = np.zeros(years_forecast_length)
    for forecasted_year in range(years_forecast_length):
        if years_of_perturbation[perturbed_year] <= years_forecast[forecasted_year]:
            T_p[forecasted_year] = T_TCRE * (
                        1 - np.exp(-k_s * (years_forecast[forecasted_year] - years_of_perturbation[perturbed_year])))
