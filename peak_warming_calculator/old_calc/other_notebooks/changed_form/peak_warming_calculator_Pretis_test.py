...
def peak_warming_calculator_Pretis(r_minus_g=0.015, g_0=0.02, beta=0.001,
    ...
    for iteration in range(num_of_iterations):
        ...
        for SCC_year in range(len(years_of_perturbation)):
            T_perturbed_ts = perturb_T_geometrically(years, T=T_ts, perturbed_year=years_of_perturbation[SCC_year],
                                                     T_TCRE_1=T_TCRE_1, k_s=k_s,
                                                     size_of_perturbation=size_of_perturbation)
            g_ts = create_consumption_growth_ts(T_0, T_ts, beta, g_0)
            g_perturbed_ts = create_consumption_growth_ts(T_0, T_perturbed_ts, beta, g_0)

            consumption_discount = g_ts[SCC_year] + r_minus_g
            discount_function = create_discount_function(years, years_of_perturbation[SCC_year],
                                                         consumption_discount=consumption_discount)

            W_ts = create_total_consumption_Pretis(years, g_ts, W_start_year=W_0)
            W_perturbed_ts = create_total_consumption_Pretis(years, g_perturbed_ts, W_start_year=W_0)

            time_series_data_Pretis = {'years': years, 'W': W_ts, 'W perturbed': W_perturbed_ts,
            ...
            SCC_adjusted = SCC - (P0_Pretis - P0_Nordhaus)
        ...
        SCC_ts_to_P_100 = np.array(SCC_ts_to_P_100)
        ...
        T_ts = emissions_to_temperature(T_0, T_TCRE_1, emissions_ts, k_s, years)

        if iteration == 0 or iteration == 1:
            peak_T = calculate_peak_T(T_0, T_TCRE_1, emissions_ts)
        else:
    ...
    if return_all_output:
        output_data = {'years': years, 'SCC': SCC_ts, 'abatement': abatement_ts,
                       'T': T_ts, 'emissions': emissions_ts, 'SCC actual': SCC_ts + (P0_Pretis - P0_Nordhaus)}
...
def calculate_peak_T(T_0, T_TCRE_1, emissions_ts):
    return sum(emissions_ts) * T_TCRE_1 + T_0
...
def emissions_to_temperature(T_0, T_TCRE_1, emissions_ts, k_s, years):
    num_of_years = len(years)
    T_ts = np.ones(num_of_years) * T_0
    for i in range(len(emissions_ts)):
        emission_size = emissions_ts[i]
        if emission_size != 0:
            T_ts = perturb_T_geometrically(years, T=T_ts, perturbed_year=years[i], T_TCRE_1=T_TCRE_1,
                                           k_s=k_s, size_of_perturbation=emission_size)
    return T_ts
