...
def varying_g_test(consumption_discount=0.035,
    ...
    # SCC_calculated, SCC_0_actual = calculate_SCC_for_perturbed_years(T_TCRE, T_forecasted_iteration,
    #                                                                  years_forecasted_length,
    #                                                                  years_forecasted, T_historical,
    ...
    T_perturbed, T_forecast_perturbed = create_T_perturbed(T_TCRE, T_forecasted_iteration, years_forecasted_length,
                                                           years_forecasted,
                                                           T_historical, k_s, perturbed_year, years_of_perturbation)

    g = create_g_forecast(g_2019, g_grad, T_2019, T_forecasted_iteration)
    g_prime = create_g_forecast(g_2019, g_grad, T_2019, T_forecast_perturbed)

    W = create_W_forecast(years_forecasted, g, last_historical_year, start_year)
    W_prime = create_W_forecast(years_forecasted, g_prime, last_historical_year, start_year)

    ## define discount function
    discount_function = create_discount_function(consumption_discount, perturbed_year, years_complete,
                                                 years_of_perturbation)
    cost = cost_of_perturbation(W, W_prime, discount_function)
    SCC = cost / (10 ** 9)
...
def read_historical_T():
    T_historical = pd.read_csv("T_gas.csv", index_col=0)
    T_historical = T_historical['CO2_best']
...
def forecast_SCC(SCC_calculated, years_forecasted, years_of_perturbation):
    ...
    # SCC_forecasted = np.exp(results.params[0] + results.params[1] * years_forecasted)

    SCC_forecasted = []

    for i in range(len(years_forecasted)):
...
#                                                                      last_historical_year, start_year, g_2019, g_grad,
#                                                                      P0_prime)
def calculate_SCC_for_perturbed_years(T_TCRE, T_forecasted_iteration, years_forecasted_length, years_forecasted, T_historical,
    ...
    for perturbed_year in range(len(years_of_perturbation)):
        ...
        T_perturbed, T_forecast_perturbed = create_T_perturbed(T_TCRE, T_forecasted_iteration, years_forecasted_length, years_forecasted,
                                                               T_historical, k_s, perturbed_year, years_of_perturbation)
    ...
    return SCC_calculated, SCC_0_actual


def create_W_forecast(years_forecasted, g, last_historical_year, start_year):
    W_2019 = 80
    W = []
    W_historical = [W_2019] * (last_historical_year - start_year + 1)
    W_forecast = []
    W_previous_year = W_historical[-1]
    for i in range(len(years_forecasted) - 1):
...
def create_g_forecast(g_2019, g_grad, T_2019, T_forecasted_iteration):
    g = g_2019 - g_grad * (T_forecasted_iteration[:-1] - T_2019)
    return g


def create_T_perturbed(T_TCRE, T_forecasted_iteration, years_forecasted_length, years_forecasted, T_historical, k_s, perturbed_year,
                       years_of_perturbation):
    T_p = np.zeros(years_forecasted_length)
    for forecasted_year in range(years_forecasted_length):
        if years_of_perturbation[perturbed_year] <= years_forecasted[forecasted_year]:
            T_p[forecasted_year] = T_TCRE * (
                        1 - np.exp(-k_s * (years_forecasted[forecasted_year] - years_of_perturbation[perturbed_year])))
    T_forecast_perturbed = T_forecasted_iteration + T_p
    T_perturbed = np.concatenate([T_historical, T_forecast_perturbed[1:]])
    return T_perturbed, T_forecast_perturbed
